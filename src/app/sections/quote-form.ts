...
export class QuoteFormSection {
  ...
  sendToWhatsApp(event: Event) {
    ...
    const phone = this.sanitizePhone(this.phone);
    const eventType = this.sanitize(this.eventType);
    const date = this.sanitize(this.date);
    const guests = this.sanitize(this.guests);
    ...
    const message = `Hola, me gustaría solicitar información sobre un evento.\n\nNombre: ${name}\nTeléfono: ${phone}\nTipo de evento: ${eventType}\nFecha del evento: ${date || 'No especificada'}\nInvitados: ${guests || 'No especificado'}`;

    window.open(`https://wa.me/522464637426?text=${encodeURIComponent(message)}`, '_blank');
  }
}
